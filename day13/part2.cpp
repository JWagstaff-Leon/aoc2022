...
struct {
    bool operator() (ListPacket* lhs, ListPacket* rhs) { return packetLessThan(lhs, rhs); };
} ListPacketComp;
...
int main(int argc, char *argv[])
{
    ...
    ListPacket* divider1 = stringToList("[[2]]");
    ListPacket* divider2 = stringToList("[[6]]");
    packets.push_back(divider1);
    packets.push_back(divider2);

    std::sort(packets.begin(), packets.end(), ListPacketComp);
